  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-[#f8fafc] text-slate-900">
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <header className="md:hidden flex justify-between items-center px-6 py-5 bg-white border-b border-slate-100 sticky top-0 z-40 pt-[calc(1.25rem+var(--safe-top,0px))]">
          <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
             <button className="w-10 h-10 rounded-full border-2 border-indigo-100 overflow-hidden active:opacity-70">
        ...
        {/* Mobile Nav - Fixed for Android */}
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-slate-100 flex justify-around items-center px-4 z-50 safe-bottom h-[calc(4.5rem+var(--safe-bottom,0px))]">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
            ...
            >
              <div className={`w-8 h-1 rounded-full mb-1 transition-all ${activeTab === tab.id ? 'bg-indigo-600' : 'bg-transparent'}`}></div>
